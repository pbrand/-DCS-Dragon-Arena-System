...
For content distribution we have chosen to go with \emph{propagating the update operation to other copies}. When using this approach, only the update operations which should be used are propagated. According to %%refrence
...
Related to pushing the updates, we have chosen to go with multicasting, because it can efficiently combined with a push-based approach to propagating updates. When the two are carefully integrated, a server that decides to push it's updates to a number of other servers simply uses a single multicast group to send its updates. %%ref book
