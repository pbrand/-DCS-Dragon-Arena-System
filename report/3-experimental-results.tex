...
To test the tolerance of the system when helpers crash, tests are run with multiple helpers server which are killed at random to simulate crashes.
It is then possible to measure the number of messages which were tried to be send from the server and the number of messages which were actually send to a helper server after one was randomly killed.
This tests needs to be run multiple times and the different outcomes should be compared in order to get a view of the number of failed send messages.
...
The number of messages which were send and received are logged and as well as the number of messages which were failed to send.
...
It is still notable that there are still some messages failing to be send. However the number of messages which failed is really small.
What is also observed, is that the failures in the helper servers is much lower that the failure in the main server.
This is probably because the main server eventually receives much more requests than each helper server.
...
And because these nodes are quite balanced, clients can reconnect faster to other helper nodes, simply because the number of helper nodes which need to reconnect is not large.
