...
public class BattleFieldViewer extends JPanel implements Runnable {
	...
	public void paint(Graphics g) {
		Unit u;
		double x = 0, y = 0;
		double xRatio = (double) this.getWidth()
				/ (double) BattleField.MAP_WIDTH;
		double yRatio = (double) this.getHeight()
				/ (double) BattleField.MAP_HEIGHT;
		double filler;
		...
		if (bufferWidth != getSize().width || bufferHeight != getSize().height
				|| doubleBufferImage == null || doubleBufferGraphics == null)
			initDB();
		...
		for (int i = 0; i < BattleField.MAP_WIDTH; i++, x += xRatio, y = 0)
			for (int j = 0; j < BattleField.MAP_HEIGHT; j++, y += yRatio) {
				...
				/* Fill the unit color */
				doubleBufferGraphics.fillRect((int) x + 1, (int) y + 1,
						(int) xRatio - 1, (int) yRatio - 1);

				/* Draw healthbar */
				 doubleBufferGraphics.setColor(Color.GREEN);
				 filler = (double)yRatio * u.getHitPoints() /
				 (double)u.getMaxHitPoints();
				 doubleBufferGraphics.fillRect((int)(x + 0.75 * xRatio),
				 (int)(y + 1 + yRatio - filler), (int)xRatio / 4,
				 (int)(filler));
